...
import { CreatePropertyListingRequest, PropertyListing } from '../../types/propertyTypes';
import {
    PROPERTY_TYPES,
    HEATING_OPTIONS,
    COOLING_OPTIONS,
...
const EditListingScreen: React.FC<EditListingScreenProps> = ({ navigation, route }) => {
    ...
    const [formData, setFormData] = useState({
        ...
        propertyType: listing.propertyType || listing.propertyTypeName || '',
        yearBuilt: listing.yearBuilt?.toString() || '',
        heatingSystems: listing.heatingSystems || [] as string[],
        coolingSystems: listing.coolingSystems || [] as string[],
        parking: listing.parking || [] as string[],
        price: listing.price?.toString() || '',
        bedrooms: listing.bedrooms?.toString() || '',
        bathrooms: listing.bathrooms?.toString() || '',
        lotSize: listing.lotSize?.split(' ')[0] || '', // Extract number from "number unit"
        lotUnit: (listing.lotSize?.split(' ')[1] as LotUnit) || 'Acres',
        squareFoot: listing.squareFoot?.toString() || '',
        description: listing.description || '',
        networth: listing.networth?.toString() || '',
        rehabEstimate: listing.rehabEstimate?.toString() || '',
        averageLeasePrice: listing.averageLeasePrice?.toString() || '',
    });
    ...
    // Initialize media items from existing listing
    useEffect(() => {
        if (listing.siteOrPropertyImages && listing.siteOrPropertyImages.length > 0) {
            const initialMediaItems: MediaItem[] = listing.siteOrPropertyImages.map(url => ({
                url: url,
                uri: url,
        ...
        if (listing.documents && listing.documents.length > 0) {
            setDocumentItems(listing.documents);
        }
    }, [listing]);
