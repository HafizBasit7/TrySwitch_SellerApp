...
// Add this reducer to mark all messages as read when chat is opened
// Add this to your chatSlice reducers
markMessagesAsRead: (state, action: PayloadAction<{ userId: string; messageIds: number[] }>) => {
  const { userId, messageIds } = action.payload;
  
  console.log(`📖 Marking specific messages as read:`, { userId, messageIds });
...
markChatAsRead: (state, action: PayloadAction<{ userId: string }>) => {
  ...
  if (state.messages[userId]) {
    ...
    console.log(`✅ Marked ${readCount} messages as read locally for user: ${userId}`);
  }
},
